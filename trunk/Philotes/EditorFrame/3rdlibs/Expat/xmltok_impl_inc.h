	  ...
	  case BT_AMP:
		...
		return tok;
	      ...
	      break;
			...
			/*
			Marcio: This doesn't make much sense, so we're removing this limitation.
			The context is clearly defined i.e. we are parsing an attribute value, so there is no point in not allowing <.
			*/
/*
	  case BT_LT:
	    *nextTokPtr = ptr;
...
{
  enum eUnnamedEnum { other, inName, inValue } state = inName;
  int nAtts = 0;
