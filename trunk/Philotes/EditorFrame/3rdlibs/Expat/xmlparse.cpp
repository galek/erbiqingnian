...
XML_ParserCreate_MM(const XML_Char *encodingName,
  ...
  memset(&position, 0, sizeof(POSITION_ST));
  ...
  nSpecifiedAtts = 0;
  dataBuf = (char*)MALLOC(INIT_DATA_BUF_SIZE * sizeof(XML_Char));
  groupSize = 0;
  groupConnector = 0;
...
int addBinding(XML_Parser parser, PREFIX *prefix, const ATTRIBUTE_ID *attId, const XML_Char *uri, BINDING **bindingsPtr)
{
  ...
  if (freeBindingList) {
    b = freeBindingList;
    if (len > b->uriAlloc) {
      b->uri = (char*)REALLOC(b->uri, sizeof(XML_Char) * (len + EXPAND_SPARE));
      if (!b->uri)
	return 0;
  ...
  for (;;) {
	...
	*eventPP = next;
	...
	if (declAttributeType) {
	  ...
	  prefix = (char*)(role == XML_ROLE_ATTRIBUTE_NOTATION_VALUE
		    ? (char*)"NOTATION("
		    : (char*)"(");
	}
