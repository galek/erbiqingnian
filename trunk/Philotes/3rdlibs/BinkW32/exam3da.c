...
//##   Returns 1, if the frame was skipped to maintain audio sync.          ##
//##                                                                        ##
//############################################################################
...
static void Show_frame( HWND window )
{
  //
...
static S32 Allocate_3D_images( )
{
  ...
  if ( ( New_foreground ) && ( New_background ) )
  {
    ...
    Decompress_frame( Bink_foreground, Image_foreground, 1, 1 );
    BinkNextFrame( Bink_foreground );

    Decompress_frame( Bink_background, Image_background, 1, 1 );
                              ...
                              WPARAM ch )
{
  switch ( ch )
  {
    ...
    case 'b': case 'B':
      Decompress_frame( Bink_background, Image_background, 0, 1 );
      break;
                    ...
                    int cmd_show )
{
  ...
  for ( ; ; )
  {
    ...
    {
      //
      // Is it time for a new Bink frame (update on the foreground schedule)
      //

      if ( ( !BinkWait( Bink_foreground ) ) || ( Play_foreground_fast ) )
      {
        Decompress_frame( Bink_foreground, Image_foreground, 0, 0 );
