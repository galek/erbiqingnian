...
char buf[1024];
	...
	PStrPrintf(buf, 1024, "%s = 0x%08X", name, value);
	msg += buf;
...
char buf[1024];
	...
	PStrPrintf(buf, 1024, "%s = %d", name, value);
	msg += buf;
...
char buf[1024];
	...
	PStrPrintf(buf, 1024, "%s = %f", name, *((float*)&value));
	msg += buf;
...
logger("RenderStates");
	...
	msg = "\t";
	DWORD _D3DRS_SRGBWRITEENABLE;
	p_pd3dDevice->GetRenderState(D3DRS_SRGBWRITEENABLE, &_D3DRS_SRGBWRITEENABLE);
	logDword(msg, "D3DRS_SRGBWRITEENABLE", _D3DRS_SRGBWRITEENABLE);
	logger(msg.c_str());
...
while (TSSNum < MaxTSSNum && !wasDisabled) {
	char buf[1024];
	PStrPrintf(buf, 1024, "	TextureStage %d", TSSNum);
	logger(buf);
	...
	msg = "\t\t";
	p_pd3dDevice->GetTextureStageState(TSSNum, D3DTSS_CONSTANT, &_D3DTSS_CONSTANT);
	logFloat(msg, "D3DTSS_CONSTANT", _D3DTSS_CONSTANT);
	logger(msg.c_str());
...
while (samplerNum < MaxTSSNum) {
	char buf[1024];
	PStrPrintf(buf, 1024, "	Sampler %d", samplerNum);
	logger(buf);
...
void dxDumpTexture(IDirect3DDevice9* p_pd3dDevice, void (*logger)(const char*), int stageNum) {
	LPD3DCBASETEXTURE pTexture = NULL;
	...
	if (hr == D3D_OK) {
		_mkdir(dirname);
		PStrPrintf(buf, 1024, "%d", stageNum);
		PStrPrintf(buf2, 1024, "%07d", frameNum);
		frameDirName = string(dirname) + "\\" + string(buf2);
		_mkdir(frameDirName.c_str());
		string filename = string(frameDirName) + "\\t" + string(buf) + ".png";
		hr = D3DXSaveTextureToFile(filename.c_str(), D3DXIFF_PNG, pTexture, NULL);
		if (hr == D3D_OK) {
...
void dxDumpRenderTarget(IDirect3DDevice9* p_pd3dDevice, void (*logger)(const char*), int targetNum) {
	...
	_mkdir(dirname);
	PStrPrintf(buf, 1024, "%d", targetNum);
	PStrPrintf(buf2, 1024, "%07d", frameNum);
	frameDirName = string(dirname) + "\\" + string(buf2);
	_mkdir(frameDirName.c_str());
	string filename = string(frameDirName) + "\\r" + string(buf) + ".png";

	hr = D3DXSaveSurfaceToFile(filename.c_str(), D3DXIFF_PNG, pInMemTarget, NULL, NULL);
		if (hr == D3D_OK) {
