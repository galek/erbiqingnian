...
PS_GREYSCALE_INPUT VS_Greyscale( VS_POSITION_TEX In )
{
...
float4 PS_Greyscale( PS_GREYSCALE_INPUT In ) : COLOR
{
	half4 vColor = tex2D( RTSampler, In.TexCoord );
	half vGrey = dot( vColor.rgb, gfFloats.rgb );
	float4 color = float4( vGrey.xxx, 1.f * gfTransition.a );
	...
	>
{
	...
	{
		COMPILE_SET_VERTEX_SHADER( vs_1_1,  VS_Greyscale() );
		COMPILE_SET_PIXEL_SHADER( _PS_1_1, PS_Greyscale() );
