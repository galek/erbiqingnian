...
#include "gearsInputJoyStickDx.h"
#include "gearsInputManagerDx.h"
#include "gearsInputKeyBoardDx.h"
#include "gearsInputMouseDx.h"

using namespace Philo;

//--------------------------------------------------------------------------------//
Win32InputManager::Win32InputManager() : GearInputManager("Win32InputManager")
{
	hWnd = 0;
	mDirectInput = 0;
...
void Win32InputManager::_initialize( ParamList &paramList )
{
	...
	if( i == paramList.end() ) 
		PH_EXCEPT(ERR_INPUT, "Win32InputManager::Win32InputManager >> No HWND found!" );

	hWnd  = (HWND)strtoul(i->second.c_str(), 0, 10);

	if( IsWindow(hWnd) == 0 )
		PH_EXCEPT(ERR_INPUT, "Win32InputManager::Win32InputManager >> The sent HWND is not valid!");
    ...
    if (FAILED(hr))	
		PH_EXCEPT(ERR_INPUT, "Win32InputManager::Win32InputManager >> Not able to init DirectX8 Input!");
...
DeviceList Win32InputManager::freeDeviceList()
{
	...
	if( keyboardUsed == false )
		ret.insert(std::make_pair(PI_Keyboard, mInputSystemName));

	if( mouseUsed == false )
		ret.insert(std::make_pair(PI_Mouse, mInputSystemName));

	for(JoyStickInfoList::iterator i = unusedJoyStickList.begin(); i != unusedJoyStickList.end(); ++i)
		ret.insert(std::make_pair(PI_JoyStick, i->vendor));
...
int Win32InputManager::totalDevices(Type iType)
{
	switch(iType)
	{
	case PI_Keyboard: return 1;
	case PI_Mouse: return 1;
	case PI_JoyStick: return joySticks;
	default: return 0;
	}
}
...
int Win32InputManager::freeDevices(Type iType)
{
	switch(iType)
	{
	case PI_Keyboard: return keyboardUsed ? 0 : 1;
	case PI_Mouse: return mouseUsed ? 0 : 1;
	case PI_JoyStick: return (int)unusedJoyStickList.size();
	default: return 0;
	}
}
...
{
	if( (iType == PI_Keyboard || iType == PI_Mouse) && vendor == mInputSystemName )
	{
		return true;
	}
	else if( iType == PI_JoyStick )
	{
		for(JoyStickInfoList::iterator i = unusedJoyStickList.begin(); i != unusedJoyStickList.end(); ++i)
			if(i->vendor == vendor)
				return true;
...
//----------------------------------------------------------------------------//
GearInputObject* Win32InputManager::createObject(GearInputManager* creator, Type iType, bool bufferMode, const std::string & vendor)
{
...
//----------------------------------------------------------------------------//
void Win32InputManager::destroyObject(GearInputObject* obj)
{
	delete obj;
}
