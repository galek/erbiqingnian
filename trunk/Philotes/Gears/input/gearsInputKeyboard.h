...
namespace Philo
{
	...
	*/
	...
	{
	public:
		KeyEvent( GearInputObject* obj, KeyCode kc, unsigned int txt ) : EventArg(obj), key(kc), text(txt) {}
		virtual ~KeyEvent() {}
	...
	*/
	class GearKeyboard : public GearInputObject
	{
	public:
		virtual ~GearKeyboard() {};
		...
		*/
		virtual void setEventCallback( GearKeyListener *keyListener ) { mListener = keyListener;}

		/**
		@remarks
			Returns currently set callback.. or 0
		*/
		GearKeyListener* getEventCallback() {return mListener;}
	...
	protected:
		GearKeyboard(const std::string &vendor, bool buffered, int devID, GearInputManager* creator)
			: GearInputObject(vendor, PI_Keyboard, buffered, devID, creator),
