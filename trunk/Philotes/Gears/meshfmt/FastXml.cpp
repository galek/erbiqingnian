...
#define DEBUG_ASSERT(x) //ph_assert(x)
...
public:
	...
	MyFastXml(Callback *c)
	{
		...
		mOpenCount = 0;
		mLastReadLoc = 0;
		for (uint32 i=0; i<(MAX_STACK+1); i++)
		{
			mStack[i] = NULL;
	...
	void indent(void)
	{
		if ( mFph )
		{
			for (uint32 i=0; i<mStackIndex; i++)
			{
				fprintf(mFph,"\t");
				fflush(mFph);
			}
	...
	char *processClose(char c, const char *element, char *scan, int32 argc, const char **argv, FastXml::Callback *iface,bool &isError)
	{
	...
	char *processClose(char *scan, FastXml::Callback *iface,bool &isError)
	{
		...
		if( !iface->processClose(close,mStackIndex,isError) )
		{
			...
			uint32 offset = (uint32)(mReadBufferEnd-scan)-1;
			uint32 readLoc = mLastReadLoc-offset;
	...
	virtual bool processXml(PxFileBuf &fileBuf,bool streamFromMemory)
	{
		releaseMemory();
		mFileBuf = &fileBuf;
	...
	{
		for (uint32 i=0; i<(mStackIndex+1); i++)
		{
			if ( !mStackAllocated[i] )
			{
				...
				if ( text )
				{
					uint32 tlen = (uint32)strlen(text);
		...
		if ( !mStreamFromMemory )
		{
			if ( scan == NULL )
			{
				uint32 seekLoc = mFileBuf->tellRead();
		...
		uint32 offset = 0;
		uint32 readLen = mReadBufferSize;

		if ( scan )
		{
			offset = (uint32)(scan - mReadBuffer );
			uint32 copyLen = mReadBufferSize-offset;
			if ( copyLen )
			{
				ph_assert(scan >= mReadBuffer);
		...
		uint32 readCount = mFileBuf->read(&mReadBuffer[offset],readLen);
		...
		while ( readCount > 0 )
		{
			...
			if ( mOpenCount < MIN_CLOSE_COUNT )
			{
				uint32 oldSize = (uint32)(mReadBufferEnd-mReadBuffer);
				...
				offset = oldSize;
				uint32 readSize = mReadBufferSize - oldSize;
	...
	const char *getError(int32 &lineno)
	{
...
private:
	...
	{
		ph_assert( mStackIndex < MAX_STACK );
