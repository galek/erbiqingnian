...
namespace Philo
{
    ...
    public:
        ...
        */
		inline Matrix3 () {}
        inline explicit Matrix3 (const scalar arr[3][3])
		{
			memcpy(m,arr,9*sizeof(scalar));
		}
        inline Matrix3 (const Matrix3& rkMatrix)
		{
			memcpy(m,rkMatrix.m,9*sizeof(scalar));
		}
        Matrix3 (scalar fEntry00, scalar fEntry01, scalar fEntry02,
                    scalar fEntry10, scalar fEntry11, scalar fEntry12,
                    scalar fEntry20, scalar fEntry21, scalar fEntry22)
		{
			m[0][0] = fEntry00;
			m[0][1] = fEntry01;
			m[0][2] = fEntry02;
		...
		*/
        ...
        inline scalar* operator[] (size_t iRow) const
		{
			return (scalar*)m[iRow];
		}
        ...
        _PhiloCommonExport friend Vector3 operator* (const Vector3& rkVector,
            const Matrix3& rkMatrix);

        // matrix * scalar
        Matrix3 operator* (scalar fScalar) const;

        // scalar * matrix
        _PhiloCommonExport friend Matrix3 operator* (scalar fScalar, const Matrix3& rkMatrix);

        // utilities
        Matrix3 Transpose () const;
        bool Inverse (Matrix3& rkInverse, scalar fTolerance = 1e-06) const;
        Matrix3 Inverse (scalar fTolerance = 1e-06) const;
        ...
        void EigenSolveSymmetric (scalar afEigenvalue[3],
    ...
    protected:
        ...
        void Tridiagonal (scalar afDiag[3], scalar afSubDiag[3]);
        bool QLAlgorithm (scalar afDiag[3], scalar afSubDiag[3]);

        // support for singular value decomposition
        static const scalar ms_fSvdEpsilon;
        ...
        static scalar MaxCubicRoot (scalar afCoeff[3]);
