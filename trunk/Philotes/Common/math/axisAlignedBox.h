	...
	public:
		...
		*/
		...
		inline AxisAlignedBox(
			...
			scalar Mx, scalar My, scalar Mz ) : mMinimum(Vector3::ZERO), mMaximum(Vector3::UNIT_SCALE), mpCorners(0)
		{
		...
		*/
		...
		inline void setMinimum( scalar x, scalar y, scalar z )
		{
			mExtent = EXTENT_FINITE;
			mMinimum.x = x;
			mMinimum.y = y;
		...
		*/
		inline void setMinimumX(scalar x)
		{
			mMinimum.x = x;
		}

		inline void setMinimumY(scalar y)
		{
			mMinimum.y = y;
		}

		inline void setMinimumZ(scalar z)
		{
			mMinimum.z = z;
		}
		...
		*/
		...
		inline void setMaximum( scalar x, scalar y, scalar z )
		{
			mExtent = EXTENT_FINITE;
			mMaximum.x = x;
			mMaximum.y = y;
		...
		used to resize only one dimension of the box
		*/
		inline void setMaximumX( scalar x )
		{
			mMaximum.x = x;
		}

		inline void setMaximumY( scalar y )
		{
			mMaximum.y = y;
		}

		inline void setMaximumZ( scalar z )
		{
			mMaximum.z = z;
		}
		...
		*/
		...
		{
            ph_assert( (min.x <= max.x && min.y <= max.y && min.z <= max.z) &&
		...
		*/
		...
		{
			ph_assert( (mExtent == EXTENT_FINITE) && "Can't get corners of a null or infinite AAB" );
			...
			if (!mpCorners)
				mpCorners = ph_new_array(Vector3,8);
		...
		*/
		...
		_PhiloCommonExport friend std::ostream& operator<<( std::ostream& o, const AxisAlignedBox aab )
		{
			switch (aab.mExtent)
			{
			case EXTENT_NULL:
		...
		*/
		...
		{
			ph_assert( (mExtent == EXTENT_FINITE) && "Can't get center of a null or infinite AAB" );
