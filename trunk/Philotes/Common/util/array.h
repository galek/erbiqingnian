...
/**
	...
	超大量数据请用Reserve，在STL同样reserve的情况下，
	Array效率是STL的3倍，也就是比不用Reserve提高了8倍效率，
	所以，尽量使用Reserve，小量数据无所谓。

	添加GetFastHashCode()用于生成FashHashCode校验，检查是否变动；

	不要求排序的Array，直接用EraseSwapIndex()；

	【关于排序的Array】

	InsertSorted在插入极大数据量时候慎用，推荐一次性插入完再用Sort()；

	排序过的Array用BinarySearchIndex；Find效率不高，无平衡性；

	一次性删除很多数据请用EraseSwap，然后Sort()一次；
...
public:
	...
	SizeT Capacity() const{return this->capacity;}
    ...
    /// unsorted array recommended ! erase element at index, fill gap by swapping in last element, destroys sorting!
    void EraseIndexSwap(IndexT index);
    /// unsorted array recommended ! erase element at iterator, fill gap by swapping in last element, destroys sorting!
	...
	///compute fastHash checksums    Added by Li
	uint32 GetFastHashCode();
...
{
    TYPE* newArray = ph_new_array(TYPE, newCapacity);
    if (this->elements)
    {
        // copy over contents
        IndexT i;
...
Array<TYPE>::Resize(SizeT num)
{
	...
	if (num > this->size)
	{
		Fill(this->size, num - this->size, TYPE());
	} 
	else
	{
		for (IndexT i = this->size; i != num; --i)
		{
...
/**
    ...
    array if necessary     Modified by Li
*/
template<class TYPE> void
Array<TYPE>::Fill(IndexT first, SizeT num, const TYPE& elm)
{
    if ( (first + num) > this->capacity )
    {
        ...
        this->GrowTo( first + num + growBy);
    }

    for (IndexT i = first; i < (first + num); ++i)
    {
        this->elements[i] = elm;
		++(this->size);
    }
...
//------------------------------------------------------------------------------
/**
	compute fastHash checksums    Added by Dinghao Li
*/
template<class TYPE> uint32
Array<TYPE>::GetFastHashCode()
{
	uint32 ret=0;
