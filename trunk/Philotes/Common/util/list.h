...
List<TYPE>::Node::~Node()
{
    ...
    ph_assert(0 == this->next);
    ph_assert(0 == this->prev);
    #endif
}
...
{
    Node* node = ph_new(Node(e));
    if (0 == iter.GetNode())
    {
        #if NEBULA3_BOUNDSCHECKS
        ph_assert((0 == this->front) && (0 == this->back));
        #endif
        this->front = node;
        this->back  = node;
    }
