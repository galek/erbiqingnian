...
{
	xtpChartPieLabelOutiside,    //Outside the pie.
...
class CXTPChartLabelsView : public CXTPChartElementView
{
public:
	CXTPChartLabelsView(CXTPChartElementView* pParentView);

public:
	void AddLabelRectangle(CXTPChartRectF& rc);

	CXTPChartPointF CalculateOffset(double lineAngle, const CXTPChartRectF& rc);

	CXTPChartDeviceCommand* CreateDeviceCommand(CXTPChartDeviceContext* pDC);

protected:
	CArray<CXTPChartRectF, CXTPChartRectF&> m_arrLabels;
};
