...
static D3DFORMAT getD3D9TextureFormat(RenderTexture2D::Format format)
{
	D3DFORMAT d3dFormat = static_cast<D3DFORMAT>(GearPlatformUtil::getSingleton()->getD3D9TextureFormat(format));
	ph_assert2(d3dFormat != D3DFMT_UNKNOWN, "Unable to convert to D3D9 Texture Format.");
	return d3dFormat;
}

static D3DTEXTUREFILTERTYPE getD3D9TextureFilter(RenderTexture2D::Filter filter)
{
	D3DTEXTUREFILTERTYPE d3dFilter = D3DTEXF_FORCE_DWORD;
	switch(filter)
	{
		case RenderTexture2D::FILTER_NEAREST:     d3dFilter = D3DTEXF_POINT;       break;
		case RenderTexture2D::FILTER_LINEAR:      d3dFilter = D3DTEXF_LINEAR;      break;
		case RenderTexture2D::FILTER_ANISOTROPIC: d3dFilter = D3DTEXF_ANISOTROPIC; break;
	}
	ph_assert2(d3dFilter != D3DTEXF_FORCE_DWORD, "Unable to convert to D3D9 Filter mode.");
	return d3dFilter;
}

static D3DTEXTUREADDRESS getD3D9TextureAddressing(RenderTexture2D::Addressing addressing)
{
	D3DTEXTUREADDRESS d3dAddressing = D3DTADDRESS_FORCE_DWORD;
	switch(addressing)
	{
		case RenderTexture2D::ADDRESSING_WRAP:   d3dAddressing = D3DTADDRESS_WRAP;   break;
		case RenderTexture2D::ADDRESSING_CLAMP:  d3dAddressing = D3DTADDRESS_CLAMP;  break;
		case RenderTexture2D::ADDRESSING_MIRROR: d3dAddressing = D3DTADDRESS_MIRROR; break;
	}
	ph_assert2(d3dAddressing != D3DTADDRESS_FORCE_DWORD, "Unable to convert to D3D9 Addressing mode.");
	return d3dAddressing;
}

D3D9RenderTexture2D::D3D9RenderTexture2D(IDirect3DDevice9 &d3dDevice, const RenderTexture2DDesc &desc) :
	RenderTexture2D(desc),
...
D3D9RenderTexture2D::~D3D9RenderTexture2D(void)
{
	if(m_d3dTexture)
	{
		m_d3dDevice.SetTexture(0, NULL);
		GearPlatformUtil::getSingleton()->D3D9BlockUntilNotBusy(m_d3dTexture);
		m_d3dTexture->Release();
	}
}

void *D3D9RenderTexture2D::lockLevel(uint32 level, uint32 &pitch)
{
	void *buffer = 0;
	if(m_d3dTexture)
	{
...
void D3D9RenderTexture2D::unlockLevel(uint32 level)
{
	if(m_d3dTexture)
	{
...
void D3D9RenderTexture2D::onDeviceLost(void)
{
...
void D3D9RenderTexture2D::onDeviceReset(void)
{
	if(!m_d3dTexture)
	{
