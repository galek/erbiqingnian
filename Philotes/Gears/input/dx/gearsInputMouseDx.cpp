...
//--------------------------------------------------------------------------------------------------//
Win32Mouse::Win32Mouse( GearInputManager* creator, IDirectInput8* pDI, bool buffered, DWORD coopSettings )
	: GearMouse(creator->inputSystemName(), buffered, 0, creator)
{
	mMouse = 0;
	mDirectInput = pDI;
...
void Win32Mouse::_initialize()
{
	...
	if( FAILED(mDirectInput->CreateDevice(GUID_SysMouse, &mMouse, NULL)) )
		PH_EXCEPT(ERR_INPUT, "Win32Mouse::Win32Mouse >> Failed to create device" );

	if( FAILED(mMouse->SetDataFormat(&c_dfDIMouse2)) )
		PH_EXCEPT(ERR_INPUT, "Win32Mouse::Win32Mouse >> Failed to set format" );
	
	mHwnd = ((Win32InputManager*)mCreator)->getWindowHandle();

	if( FAILED(mMouse->SetCooperativeLevel(mHwnd, coopSetting)) )
		PH_EXCEPT(ERR_INPUT, "Win32Mouse::Win32Mouse >> Failed to set coop level" );
	
	if( FAILED(mMouse->SetProperty(DIPROP_BUFFERSIZE, &dipdw.diph )) )
		PH_EXCEPT(ERR_INPUT, "Win32Mouse::Win32Mouse >> Failed to set property" );

	HRESULT hr = mMouse->Acquire();
	if (FAILED(hr) && hr != DIERR_OTHERAPPHASPRIO)
		PH_EXCEPT(ERR_INPUT, "Win32Mouse::Win32Mouse >> Failed to aquire mouse!" );
}
