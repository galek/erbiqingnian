...
//--------------------------------------------------------------------------------------------------//
Win32JoyStick::Win32JoyStick( GearInputManager* creator, IDirectInput8* pDI,
	bool buffered, DWORD coopSettings, const JoyStickInfo &info )
	: GearJoyStick(info.vendor, buffered, info.devId, creator)
{
	mDirectInput = pDI;
...
void Win32JoyStick::_initialize()
{
	...
	if(FAILED(mDirectInput->CreateDevice(deviceGuid, &mJoyStick, NULL)))
		PH_EXCEPT(ERR_INPUT, "Win32JoyStick::_initialize() >> Could not initialize joy device!");

	if(FAILED(mJoyStick->SetDataFormat(&c_dfDIJoystick2)))
		PH_EXCEPT(ERR_INPUT, "Win32JoyStick::_initialize() >> data format error!");

	HWND hwin = ((Win32InputManager*)mCreator)->getWindowHandle();

	if(FAILED(mJoyStick->SetCooperativeLevel( hwin, coopSetting)))
		PH_EXCEPT(ERR_INPUT, "Win32JoyStick::_initialize() >> failed to set cooperation level!");

	if( FAILED(mJoyStick->SetProperty(DIPROP_BUFFERSIZE, &dipdw.diph)) )
		PH_EXCEPT(ERR_INPUT, "Win32Mouse::Win32Mouse >> Failed to set buffer size property" );
...
BOOL CALLBACK Win32JoyStick::DIEnumDeviceObjectsCallback(LPCDIDEVICEOBJECTINSTANCE lpddoi, LPVOID pvRef)
{
	...
	if (FAILED(_this->mJoyStick->SetProperty(DIPROP_RANGE, &diprg.diph)))
		PH_EXCEPT(ERR_INPUT, "Win32JoyStick::_DIEnumDeviceObjectsCallback >> Failed to set min/max range property" );
...
void Win32JoyStick::capture()
{
	...
	for(unsigned int i = 0; i < entries; ++i)
	{
		...
		if( diBuff[i].uAppData != 0xFFFFFFFF && diBuff[i].uAppData > 0 )
		{
			...
			ph_assert( axis >= 0 && axis < (int)mState.mAxes.size() && "Axis out of range!");
...
//--------------------------------------------------------------------------------------------------//
GearInputInterface* Win32JoyStick::queryInterface(GearInputInterface::IType type)
{
	//Thought about using covariant return type here.. however,
	//some devices may allow LED light changing, or other interface stuff

	if( ff_device && type == GearInputInterface::ForceFeedback )
		return ff_device;
	else
		return 0;
}
